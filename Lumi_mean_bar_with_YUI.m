...
% Switch to left Y-axis (primary axis)
yyaxis left
...
% Convert tick values to percentages
percentTicks = yTicks * 100;
tickLabels = arrayfun(@(x) sprintf('%.0f%%', x), percentTicks, 'UniformOutput', false);

% Update Y-axis properties
set(ax, 'YTick', yTicks, ...               % Keep original tick positions
         'YTickLabel', tickLabels, ...      % Apply percentage labels
         'YLim', yLimits);                  % Maintain original limits

% Update Y-axis label
ylabel(ax, ['Normalized RLUs level' ...
    '(DMSO as 100%)'], ...
    'FontSize', 16, ...
    'FontWeight', 'bold', ...
    'Color', 'k');
% Hide right Y-axis for cleaner appearance
yyaxis right
